...
This unit uses R packages `learnr` [@Schloerke2020], `phyclust` [@Chen2011] and `abc` [@Csillery2012], function to read *ms* output from `readms.output.R` script distributed with `ms` [@Hudson2002], and functions to calculate summary statistics from *ms* simulations in file `ms_sumstats.R` are modified from [@Stadler2009a; @Stadler2009b].
...
n <- h+t
num_of_sims = _____
h_prime <- rbinom(n=num_of_sims,size=n,prob=p)
_____
```
...
source("src/coin_toss.R")
```


```{r likelihood_function, exercise=TRUE}
h = 4; t = 6; num_of_sims = 1000
p = seq(0,1,0.01)
l_profile_true   <- flip_coin_likelihood(p,h,t)
l_profile_approx <- flip_coin_likelihood_approx(p,h,t,num_of_sims)
...
h=4;t=6;a=1;b=1;num_of_sims=1000
res <- flip_coin_posterior_approx(h,t,a,b,num_of_sims)
...
_____(octomanati_data)
_____(rinocaracol_data)
```
...
sample_size = 50
num_of_sims = 10
theta_prime <- 10^runif(num_of_sims,min=-1,max=2)
...
ref_table <- data.frame(theta=theta_prime,
                        ...
                        TD=TD_prime,
                        FLD=FLD_prime)
saveRDS(ref_table,file="data/ref_table_1.RDS")
```
...
saveRDS(target,file="data/target.RDS")
```
...
num_of_sims <- 10000 # use a subset of simulation from a total of 100000
