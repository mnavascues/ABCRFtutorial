...
Estimate $\theta$ for octomanati and rinocaracol.
...
hist(x = log10(ref_table[,"theta"]),
     ...
     xlab = expression(log(theta)),
     ylab = "probability density")
wtd.hist(x = log10(octomanati_result$adj.values),
         breaks = seq(-1.5,2,0.05),
         col = "yellow", freq=FALSE, add=TRUE,
         weight = octomanati_result$weights)
wtd.hist(x = log10(rinocaracol_result$adj.values),
         breaks = seq(-1.5,2,0.05),
         col = "orange", freq=FALSE, add=TRUE,
         weight = rinocaracol_result$weights)
