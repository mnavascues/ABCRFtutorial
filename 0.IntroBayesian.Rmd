...
```{r color, include=TRUE, eval=TRUE}
Orange         <- "#E69F00"
Sky_Blue       <- "#56B4E9" 
Bluish_Green   <- "#009E73"
Yellow         <- "#F0E442"
...
plot(x = likelihood_profile$_____,
     ...
     xlab = "p", ylab = "Likelihood", type = "l", col = Bluish_Green)
```
...
```{r get_maxL, eval=TRUE, echo=FALSE}
maxL <- max(likelihood_profile$l)
```
...
plot(x = likelihood_profile$p,
     y = likelihood_profile$l,
     xlab = "p", ylab = "Likelihood", type = "l", col = Bluish_Green)
abline(v=p_hat, col=Bluish_Green)
abline(v=CI95[1], col=Bluish_Green,lt=2)
abline(v=CI95[2], col=Bluish_Green,lt=2)
abline(v=0.5, col=Sky_Blue)
...
abline(v=quant[1], col=Vermillion)
abline(v=quant[2], col=Vermillion,lt=2)
abline(v=quant[3], col=Vermillion,lt=2)
abline(v=0.5, col=Sky_Blue)
```
...
Until this point we have considered that the binomial model describes well the reality of our coin. However, in addition to heads and tails there are other possible outcomes when tossing a coin, e.g. falling and staying on its side.
